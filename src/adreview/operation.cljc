(ns adreview.operation
  "OperationActor -- one advertising-review operation = one supervised
  actor run, expressed as a langgraph StateGraph. The advisor
  (AdReview-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Ad-Review Compliance Governor (:govern) and
  ...
  One graph run = one advertising-review operation. No unbounded inner
  loop.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor.
  `:review/submit`/`:campaign/publish` ALWAYS reach this node when the
  governor is clean -- see `adreview.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [adreview.adreviewllm :as adreviewllm]
            [adreview.governor :as governor]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (adreviewllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (adreviewllm/-advise advisor store request)]
            {:proposal p :audit [(adreviewllm/trace request p)]})))
